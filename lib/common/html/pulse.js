...
function changeVisibility(category, show){
    ...
    function addConfiguration(json, cat){
        ...
        logdebugtags = getFieldValue(cat, 'logdebugtags', "input")
    ...
    function restartScheduler(e){
        e.preventDefault(); //this prevents default action
        getJSON("/api/restartSchedule",(err)=>{ 
            if(err == null)
                window.alert("Scheduler restarted");
        });
    }
    ...
    const allPattern = "^(((A-A?|A),)*(A-A?|A)|*)$" // A stands for either hourPatternA or hourPatternB
        ...
        if( json ){
            if( json.network != undefined){
                ...
                setFieldValue( 'logdebugtags',json.network.logdebugtags);
            }else{
...
function updateLastCount(){
    ...
    getJSON("/api/status",(err,result)=>{
        ...
        if(err == null && result != null){
            divErrorObj = document.querySelector("div[id='errors']")
            let childCount=0;
            let errorNode = divErrorObj.querySelector("p")
            while (errorNode) {
                found = result.filter(
                    e=>e.to == errorNode.getAttribute("id")
                );
                if( found.length == 1 )  
                    childCount++;
            ...
            updateErrors = true
            if( childCount != result.length)
                while(divErrorObj.firstChild)
                    divErrorObj.removeChild(divErrorObj.lastChild);
            else 
                updateErrors = false
            result.forEach( s=>{
                if( s.lastPublished != undefined){
                    divErrorObj.textContent = "Last published at " + dateFormat.format( s.lastPublished )
                }else
                if(s.error != undefined ){
                        if( divErrorObj && updateErrors){
                            const fragment = document.createDocumentFragment();
                            const pObj = document.createElement("p");
                           
                            pObj.textContent =  dateFormat.format( s.to ) + " " +  s.error;
                            pObj.setAttribute("id", s.to);
                            fragment.append(pObj)
                            divErrorObj.appendChild(fragment)
                        }
                }
                else{
                    ...
                    if( s.lastSecond != 0 ){
                        ...
                        if( minutes == 0 )
                            ...
                            divObj.innerHTML= "<p style='color:red'>No pulse since " + Math.trunc((Date.now()/1000 - s.lastSecond)/60/60 ) + " or more hours</p>"
                    }
                    else
                        divObj.innerText = ""
                }
...
document.addEventListener("DOMContentLoaded", (event) => {
    updateLastCount();
    var intervalId = window.setInterval(updateLastCount, 5000);
  ...
  document.getElementById('restartScheduler').addEventListener('click', restartScheduler);
